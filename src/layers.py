...
class ModelEncoder(nn.Module):
	...
	def forward(self, x, padding_mask, test=False):
		for i in range(self.num_blocks):
			x = self.blocks[i](x, padding_mask, i * (self.num_conv + 2) + 1, self.num_blocks * (self.num_conv + 2),
			                   test=test)
		return x
...
class OutputLayer(nn.Module):
	...
	def __init__(self, in_dim, out_dim=1):
		...
		self.W11 = nn.Linear(in_dim, in_dim)
		self.W12 = nn.Linear(in_dim, out_dim)

		self.W21 = nn.Linear(in_dim, in_dim)
		self.W22 = nn.Linear(in_dim, out_dim)

	def forward(self, m0, m1, m2, attn, mask):
		concat1 = torch.cat([m0, m1, attn], dim=2)
		concat2 = torch.cat([m1, m2, attn], dim=2)
		lin_out1 = self.W12(F.relu(self.W11(concat1)))
		lin_out2 = self.W22(F.relu(self.W21(concat2)))
...
class ConditionalOutputLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim=1):
        super(ConditionalOutputLayer, self).__init__()
        half_dim = in_dim // 7
        self.V1 = nn.Linear(in_dim, half_dim)
        self.v1 = nn.Linear(half_dim, 1)
        self.V2 = nn.Linear(in_dim, half_dim)
        self.v2 = nn.Linear(half_dim, 1)
        self.W1 = nn.Linear(in_dim, half_dim)
        self.W = nn.Linear(half_dim, half_dim)
        for params in [self.V1, self.V2, self.W1, self.v1, self.v2, self.W]:
            nn.init.xavier_uniform_(params.weight)

    def forward(self, m0, m1, m2, att_reuse, mask):
        batch_size, seq, hidden = m0.size()
        H_r = torch.cat([m0, m1, m2, att_reuse], dim=2)
        F_start = F.relu(self.V1(H_r))
        beta_start = masked_softmax(self.v1(F_start).squeeze(), mask, dim=1, log_softmax=True)
        h = torch.sigmoid(self.W1(torch.bmm(beta_start.unsqueeze(1), H_r).squeeze(1)))
        F_end = F.relu(self.V2(H_r) + (self.W(h)).unsqueeze(1).expand(-1, seq, -1))
        beta_end = masked_softmax(self.v2(F_end).squeeze(), mask, dim=1, log_softmax=True)

        return beta_start, beta_end

class GRUCell(nn.Module):
	"""
	An implementation of GRU cell that assumes the input and hidden size are the same
	"""
	def __init__(self, hidden_size):
		super(ConditionalOutputLayer, self).__init__()
		h_init = torch.Tensor(1, hidden_size)
		nn.init.normal_(h_init)
		self.h_init = nn.Parameter(h_init, requries_grad = False)
		self.h_weight = nn.Linear(hidden_size, hidden_size)
		self.reset
...
# F_start = torch.tanh(self.V1(H_r))
#         beta_start = masked_softmax(self.v1(F_start).squeeze(), mask, dim=1, log_softmax=True)
#         # Lstm
#         h, _ = self.ans_lstm(torch.bmm(beta_start.unsqueeze(1), H_r).squeeze(1))
#         F_end = torch.tanh(self.V2(H_r) + (self.W(h)).unsqueeze(1).expand(-1, seq, -1))
#         beta_end = masked_softmax(self.v2(F_end).squeeze(), mask, dim=1, log_softmax=True)