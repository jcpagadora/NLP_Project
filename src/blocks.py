...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, inp_dim, num_conv=2, kernel=7, num_heads=1, dropout=0.1):
        ...
        self.feedforward = PointwiseFeedforward(input_channel=inp_dim, output_channel=inp_dim)
...
class MHA(nn.Module):
    """
    multiheaded attention with mask adapted from hw 5 Causal MHA + gated attention
    ...
    def __init__(self, n_embd, n_head, attn_pdrop=0.1, resid_pdrop=0.1):
        ...
        self.proj1 = nn.Linear(4*n_embd//n_head, n_embd//n_head)
        self.proj2 = nn.Linear(4*n_embd//n_head, n_embd//n_head)
        # regularization
    ...
    def forward(self, x, mask):
        ...
        k = F.relu(self.key(x)).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        q = F.relu(self.query(x)).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        ...
        # fusion
        intermediate = F.relu(self.proj1(torch.cat([v, y, v*y, v-y], dim=-1)))
        gate = torch.sigmoid(self.proj2(torch.cat([v, y, v*y, v-y], dim=-1)))
        y = gate * intermediate + (1 - gate) * v
